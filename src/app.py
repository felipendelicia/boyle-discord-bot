...
import os
from modules import chemistryCommands
from modules import mathCommands
from modules import physicsCommands
from modules import othersCommands

bot = commands.Bot(
    command_prefix="*",
    description="Un bot que te facilita las tareas",
    help_command=None
    )
...
async def molecular_mass(ctx, formule):
    await ctx.send(chemistryCommands.molecularmass(formule))
...
async def balance_equations(ctx, *args):
    ...
    await ctx.send(chemistryCommands.balancereactions(reactives, products))

@bot.command()
async def quadratic_function(ctx, a, b=0, c=0):
    await ctx.send(mathCommands.quadratic_function(a, b, c))
...
async def convert_units(ctx, *args):
    await ctx.send(othersCommands.convert_units(list(args)))
...
async def concentration(ctx, *args):
    ...
    call_to_the_function = chemistryCommands.concentration(
        arguments_dictionary)
...
async def trition(ctx, cpattern, vpattern, vsample, nOH, nH, baseacid):
    await ctx.send(chemistryCommands.trition(cpattern, vpattern, vsample, nOH, nH, baseacid))
...
async def ideal_gases(ctx, *args):
    ...
    output = physicsCommands.ideal_gases(params_dic, out_unit)
